...
import { axioslib } from '@/lib/axioslib'; // Import axioslib
...
const Teacher_EditAttendance: React.FC = () => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            const response = await axioslib.post(`/api/user/createattend/${classID}`, attendanceData);
                ...
                <div className='w-1/3 flex flex-col text-salate-1000'>
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className='w-60 md:w-72 lg:w-96 2xl:w-auto bg-content-light rounded-b-2xl text-xs md:text-base lg:text-lg pt-4 p-4 md:p-8 lg:p-12'>
                            ...
                            <p className='mt-8 text-xs md:text-sm text-primary'>
                                After 15 minutes from the Time Start, the status will be <span className='font-black'>Late</span>
                            </p>
                            <p className='mt-4 mb-8 text-xs md:text-sm text-primary'>
                                And after more than 30 minutes, the status will be <span className='font-black text-bookmark1'>Absent</span>
                            </p>
                            <button
                                type="submit"
