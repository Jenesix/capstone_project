    ...
    return (
        <div className="min-h-screen flex flex-col mt-12 w-full px-4 sm:px-8 pb-6">
            ...
            <h2 className="text-salate-1000 text-center font-bold text-2xl max-w-48 mb-5">{countUser} People</h2>
                <table className= "text-salate-1000 font-semibold rounded-md text-center min-w-min bg-white ">
                    <thead>
                        <tr className=' h-24 bg-primary-light text-white text-lg px-6 py-3 tracking-wider'>
                            <th className="font-black rounded-tl-4xl  min-w-96 ">Student</th>
                            <th className="font-black border-x border-salate-1000 min-w-72">Faculty</th>
                            <th className="font-black border-x border-salate-1000 min-w-72">Department</th>
                            <th className="font-black rounded-tr-4xl min-w-72">Major</th>
                        </tr>
                    </thead>
                    <tbody>
                    {User.map((entry, index) => (
                            <tr key={index} className={index % 2 === 0 ? "bg-content-light h-28 " : "bg-white h-28"}>
                                <td className="border border-salate-1000 px-6 py-4 whitespace-no-wrap font-extrabold">
                                ...
                                <td className="border border-salate-1000 px-6 py-4 whitespace-no-wrap">{entry.FacultyID}</td>
                                <td className="border border-salate-1000 px-6 py-4 whitespace-no-wrap">{entry.DepartmentID}</td>
                                <td className="border border-salate-1000 px-6 py-4 whitespace-no-wrap">{entry.MajorID}</td>
                            </tr>
                        ))}
