...
const QnACard: React.FC<QnACardProps> = ({bg_post, boardID, board_title, board_desc, postimage, time, size_card, height_detail, size_image, user_id, profileImage, firstname, lastname, size_profile, size_divtext, size_nameuser, size_iduser}) => {

    return (
        <Link href={`QnABoard/${boardID}`}>
        <div className={`${bg_post} shadow-2xl rounded-lg p-4 pl-12 pt-8 mb-8 mr-8 ${size_card} text-salate-1000 overflow-hidden`}>



            <div className='flex flex-row mb-4'>
                <h1 className='font-bold text-xl text-primary truncate'>{board_title}</h1>
                <SlOptionsVertical className='ml-auto size-4'/>
            </div>
