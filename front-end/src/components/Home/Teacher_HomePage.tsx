...
interface Teacher_HomepageProps {
  role: 'teacher' | 'student';
  enrolledClasses: { classes: Class[] };
}

const Teacher_Homepage: React.FC<Teacher_HomepageProps> = ({ role, enrolledClasses }) => {
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 lg:mt-6 mt-12 gap-2 md:gap-5 xl:gap-8 mx-16 2xl:mx-36'>
        {filteredClasses.length > 0 ? (
          filteredClasses.map((classData, index) => {
            return (
              <Teacher_Card key={classData._id} classData={classData} role={role} />
            );
          })
        ) : (
